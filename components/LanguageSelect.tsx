...
export const LanguageSelect = () => {
    ...
    return (
        ...
        className="flex z-99 gap-15 justify-center items-center relative uppercase fhd:pb-10 fhdv:pb-0">
            <div className="flex gap-2 items-center cursor-pointer" onClick={() => (setShowSelect(!showSelect))}>
                ...
                <div className="text-primary fhd:text-sm fhdv:text-xl">( {locale} )</div>
            ...
            <div className={`transition-discrete duration-300 flex flex-col gap-1 absolute -top-26 bg-quaternary rounded-full overflow-hidden ${showSelect ? "max-h-100 p-2 border-primary border" : "max-h-0"}`}>
                ...
                return (
                    <div className="gap-15 items-center text-primary cursor-pointer fhd:text-sm fhdv:text-xl" key={index}>
                        <Link onClick={() => {
                            setShowSelect(false)
